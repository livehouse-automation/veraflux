...
--
	...
	local function sendVeraFluxData()
		...
		veraFluxDebugLog("LiveHouseInflux: Payload is: " .. LINE_PROTOCOL) 
		veraFluxDebugLog("LiveHouseInflux: Submitting payload to InfluxDB at: " .. INFLUX_URL) 
		...
		veraFluxDebugLog("LiveHouseInflux: InfluxDB server replied: " .. code) 
	...
	local function sanitizeMeasurement(measurement)
		-- not currently used, serviceId's are all valid.
		-- firstly convert to string
	...
	local function processDevice(deviceId, d, svcsTbl)
		...
		veraFluxDebugLog("LiveHouseInflux: Processing device " .. tostring(deviceId) .. ": " .. tostring(d.device_type)) 
		...
		for serviceId, serviceTable in pairs(svcsTbl) do
			...
			if luup.device_supports_service(serviceId, deviceId) then
				...
				if submitLineProtocol then
					...
					newLineProtocol = tostring(serviceId) .. lineProtocolDeviceTags .. newLineProtocol .. "\n"
					veraFluxDebugLog("LiveHouseInflux: New Line Protocol: " .. newLineProtocol) 
	...
	function veraFluxWatchedVariableCallback(lul_device, lul_service, lul_variable, lul_value_old, lul_value_new)
		...
		veraFluxDebugLog("LiveHouseInflux: watchedVariableCallBack: Called with lul_device='" .. tostring(lul_device) .. "', lul_service='" .. tostring(lul_service) .. "', lul_variable='" .. tostring(lul_variable) .. "', lul_value_old='" .. tostring(lul_value_old) .. "', lul_value_new='" .. tostring(lul_value_new))
		...
		if (enable == "1") then
			...
			veraFluxDebugLog("LiveHouseInflux: watchedVariableCallBack: Finished with device " .. tostring(lul_device) .. ", service " .. tostring(lul_service) .. ", variable " .. lul_variable)
		else
					...
					for serviceId, serviceTable in pairs(servicesTable) do
						...
						if luup.device_supports_service(serviceId, deviceId) then
							...
							for i, field in ipairs(servicesTable[serviceId]['watchedFields']) do
								...
								veraFluxDebugLog("LiveHouseInflux: setupCallbacks: Registering variable_watch callback for: serviceId='" .. tostring(serviceId) .. "', variable='" .. tostring(field) .. "', deviceId='" .. tostring(deviceId) .. "'")
    ...
    function veraFluxOnTimer()
      ...
      if (enable == "1") then
		...
		veraFluxDebugLog("LiveHouseInflux: STARTING SCHEDULED COLLECTION")
		addAllDeviceValues() -- cycle through all non-invisible devices and collect values for supported devices
		sendVeraFluxData() -- send all generated line protocol to influx
		veraFluxDebugLog("LiveHouseInflux: FINISHED SCHEDULED COLLECTION") -- may want to comment this out when this script is working	
